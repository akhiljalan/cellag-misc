...
def main(): 
    ...
    m_range = [5, 10, 15, 20]
    delta_scale_range = [0.0, 0.1, 0.5]
    occlusion_levels = [0.0, 0.01, 0.1, 0.5]
    # k_levels = [10, 20, 30]
    num_eigs_to_test = 20
    ...
    for m_val, delta_val, occ_val in param_sets: 
        ...
        objective = lambda eta_arr: gl.objective_with_params_sparse(eta_arr, validation_mat, 
                                                                    sample_mats, delta, num_eigs_included = num_eigs_to_include)
        bfgs_options = {
            'iprint': 5, 
            'maxiter': 500, 
        }
        ...
        cur_time = get_cur_time_str()
        print('END TEST at time {}'.format(cur_time))
        print_ln()
